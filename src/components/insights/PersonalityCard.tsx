...
export default function PersonalityCard({ data }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h3 className="text-sm font-bold text-slate-800">Personality Profile</h3>
        <p className="text-xs text-slate-400">Big Five OCEAN · inferred from learning patterns</p>
      </div>
      ...
      {data.gentle_reminder && (
        <div className="bg-amber-50/70 border border-amber-100 rounded-xl p-3 text-xs text-amber-800 leading-relaxed">
          💡 {data.gentle_reminder}
        </div>
      )}
    </div>
